import { getConfirmOrderDetail, confirmOrder, orderPay, saveAddressDetails, saveIdentityInfo, updateAddress } from '../../api/api.js'
...
Page({
  ...
  _closePayPanelEnv() {
    ...
    if(this.data.postType == "2" && this.data.payStatus){
      wx.redirectTo({
        url: `/pages/groupShoping/groupShoping?orderId=${this.data.orderId}`,
      })
      return
    }
    ...
    getConfirmOrderDetail(params).then(res => {
      ...
      if (res.data.code === '1') {
        this.setData({
          ...
          // certificationId: res.data.addressInfo.certificationId,
          identityName: this.data.identityName || res.data.addressInfo.identityName,
          identityNo: this.data.identityNo || res.data.addressInfo.identityNo,
          realIdentityNo: this.data.realIdentityNo || res.data.addressInfo.realIdentityNo,
          hide: true,
  ...
  saveAddressEnv(e) {
    ...
    params = {
      ...
      identityName: this.data.identityName,
      identityNo: this.data.realIdentityNo || '',
      identityFrontRelative: this.data.frontImg,
      identityBackRelative: this.data.backImg,
      // isDefault : true
      addressId : this.data.addressInfo.addressId,
  ...
  getIdNum(e){
    let realIdentityNo = e.detail.value
    this.setData({
  ...
  _uploadFile(file) {
    ...
    return new Promise((resolve, reject) => {
      wx.uploadFile({
        url: `${baseUrl}/clt/user/webUploadImage.msp`,
        filePath: file,
        name: 'file',
        formData: {
          name: 'file'
  ...
  _checkForm(e) {
    ...
    let realIdentityNo = this.data.realIdentityNo
  ...
  goCeleAdd(){
    ...
    // if (this.data.addressInfo.isValidIdentity=="0"){
    //   edit = '0'
    // }else{
     let edit = '1'
      wx.setStorageSync('userInfo', this.data.addressInfo)
    // }

    wx.navigateTo({
      url: `/pages/celebrityAdd/celebrityAdd?edit=${edit}`,
    })
  ...
  onLoad(options) {
    ...
    this.data.collageId = options.collageId
    this.data.isSharePage = options.isSharePage == undefined? "0" : options.isSharePag
    this.setData({
      number: this.data.number || '',
      postType: this.data.postType || '',
      productId: this.data.productId || '',
  ...
  onHide: function () {
    ...
    console.log('-----------------', getApp().globalData.goToDetail)
    console.log('hide111')
  ...
  onUnload: function () {
    ...
    //     getApp().globalData.goToDetail = false
    //   } else {
    //     getApp().globalData.goToDetail = true
    //   }
    // }
