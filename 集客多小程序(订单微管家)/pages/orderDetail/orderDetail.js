...
Page({
  ...
  data: {
    ...
    statusName: ["待付款", "待发货", "已发货", "交易完成", '交易关闭'],
    /*对应状态0-8*/
  ...
  _getOrderDetail(orderId) {
    ...
    getOrderDetail(params).then(res => {
      ...
      this.setData({
        ...
        isCollageBuy: this.data.orderInfo.isCollageBuy,
        showCollageDetail: this.data.orderInfo.showCollageDetail,
        headImageList: res.data.headImgList,
        status: Number(this.data.orderInfo.status),
      }, () => {
        console.log(this.data.orderInfo.status)
      })
