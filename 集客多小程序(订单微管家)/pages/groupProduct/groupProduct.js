...
Page({
  ...
  innerConfirmEnv() {
    if (this.data.curGoods.isSelectedFinish) {
      ...
      console.log(this.data.curGoods)
  ...
  selectSpecEnv(e) {
    console.log(e)
    console.log('选择的是第几个商品', e.target.dataset.index)
    this.setData({
      selectedIndex: e.target.dataset.index
    })
    console.log(e.currentTarget.dataset.goods) 
    let localPrimaryNumber = []
    e.currentTarget.dataset.goods.productSpecList.forEach((data,index) => {
      localPrimaryNumber.push(data.secondarySpecName)
    })
    localPrimaryNumber = this.uniq(localPrimaryNumber)
    console.log(localPrimaryNumber)
    this.setData({
      localPrimaryNumber
    })
  ...
  setSelectedSpec(e, specType) {
    ...
    // 修改显示价格
    this.data.curGoods.price = selectSpec.price
    // console.log(this.data.curGoods)
    // let selectedIndex = this.data.selectedIndex
    // this.data.goodsList[selectedIndex].price = selectSpec.price
    // this.setData({
    //   goodsList: this.data.goodsList
    // }, () => {
  ...
  _getGroupProductList() {
    ...
    getGroupProductList(params).then(res => {
      ...
      // let currentSpecList = res.data.dataList.
      this._hideLoading()
      ...
      if (res.data.code === '1') {
        ...
        goodsList.forEach(goods => {
          ...
          // secondSpecNumber.push(goods.secondarySpecName)
        })
        // console.log(secondSpecNumber)
        this.setData({
          goodsList: goodsList
        })
      }
