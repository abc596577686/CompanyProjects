...
JyoComponent.create = function (name, id, page) {
  ...
  if (typeof component === 'undefined') throw "无效的组件名：" + name;
  ...
  page.onUnload = function (e) {
    ...
    if (typeof oldUnloadFn === 'function') oldUnloadFn.call(page, e);
  };
...
JyoComponent.register = function (name, obj) {
  ...
  if (obj.prototype instanceof Base) {
    this._components[name] = obj;
    // console.log(`组件 "${name}" 已注册`);
  } else {
    // console.error(`非法组件 "${name}" 注册失败`);
  }
};
...
// 排列滚动视图组件
import sortListView from 'SortListView/SortListView';
JyoComponent.register('SortListView', sortListView);
