...
// 保存使用经销商修改（总监）
export const updateBdUserMemberNumber = params => {
  return Http(`${localUrl}clt/user/updateBdUserMemberNumber`, params, 'post');
};

// 启用分配名额
export const allocateUserMember = ( params, method ) => {
  return Http(`${localUrl}clt/customer/allocateUserMember`,  params, 'GET');
};

// 资源分配记录
export const bdMemberQuotaList = ( params, method ) => {
  return Http(`${localUrl}clt/bdMemberQuotaList`,  params, method);
};

// 资源分配记录
export const bdMemberQuotaAllocationList = ( params, method ) => {
  return Http(`${localUrl}clt/bdMemberQuotaAllocationList`,  params, method);
};

// 资源分配记录
export const userProfitRecordList = ( params, method ) => {
  return Http(`${localUrl}clt/userProfitRecordList`,  params, method);
};

// 申请对接记录
export const dockRecordList = ( params, method ) => {
  return Http(`${localUrl}clt/dockRecordList`,  params, method);
};

// 图片上传
export const webUploadImage = params => {
  return util.ajax.post(`${localUrl}clt/upload/webUploadImage`, params).then(res => res.data)
}

// 发起对接申请
export const applyDock = ( params, method ) => {
  return Http(`${localUrl}clt/dock/applyDock`,  params, 'GET');
};

// 申请对接处理 
export const checkDock = ( params, method ) => {
  return Http(`${localUrl}clt/dock/checkDock`,  params, method);
};
