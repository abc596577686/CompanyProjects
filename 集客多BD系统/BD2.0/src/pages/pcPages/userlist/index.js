...
import { userList, channelList, getRegionList, getroleList, saveUser, updateBdUserMemberNumber, getUserInfo } from '../../../axios/api';
...
export default class GiftPack extends React.Component {
      ...
      if (res.data.code == 1) {
        if (res.data.data) {
          this.setState({
            ...
            console.log('当前等级',this.state.superAccount)
          })  
        }
      }
    })
  ...
  _setdata(adminlist) {
    ...
    adminlist.map((user, i) => {
      mockTableListData.push({
        ...
        surplusNumber: user.surplusNumber,
        jkdUserId: user.jkdUserId
      })
    })
    this.setState({
      adminlist,
  ...
  // 修改试用名额
  handleSurplusNumber = (data,e) => {
    // console.log(e.target.value)
    console.log(data)
    console.log(e.target.value)
    if (e.target.value <= data) {
      // this.setState({
        // countNum: Number(data)
      // })
      message.error('修改数额需大于原始数额 '+data);
    }
    this.setState({
  ...
  // 保存试用经销商名额修改
  surehandleSurplusNumber = (data) => {
    // console.log(data.jkdUserId)
    // console.log(this.state.countNum)
    if (this.state.countNum == '' || this.state.countNum == undefined) {
      message.warning('请填写修改内容');
      return
    }
    let params = {
      jkdUserId: data.jkdUserId,
      surplusNumber: this.state.countNum
    }
    updateBdUserMemberNumber(params, 'POST').then(res => {
      if (res.data.code == 1) {
        message.success(res.data.msg);
        this.setState({
          countNum: ''
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              {
                this.state.superAccount == false?
                <Input
                  style={{ width: '100px', textAlign: 'center', marginRight: '20px' }}
                  value={record.surplusNumber}
                />
                :null
              }
              {
               this.state.superAccount == true?
                <Input
                  style={{ width: '100px', textAlign: 'center', marginRight: '20px' }}
                  defaultValue={record.surplusNumber}
                  onChange={this.handleSurplusNumber.bind(this, record.surplusNumber)}
                />
                :null
              }
              {
                this.state.superAccount == true?
                <Button onClick={this.surehandleSurplusNumber.bind(this, record)}>修改</Button>
                :null
              }
            </div>
          )
