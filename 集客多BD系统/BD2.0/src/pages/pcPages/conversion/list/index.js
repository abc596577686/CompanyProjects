...
export default class GiftPack extends React.Component {
  ...
  state = {
    ...
    teamColumns: [
      ...
      {
        title: '昨日注册',
        dataIndex: 'yesterdayRegisterNumber',
        key: 'col04',
        width: 70,
      },
      {
        title: '昨日晋升',
        dataIndex: 'yesterdayPromoteNumber',
        key: 'col05',
        width: 70,
      },
      {
        title: '昨日续费',
        dataIndex: 'yesterdayRenewNumber',
        key: 'col06',
        width: 70,
      },
      {
  ...
  setTeamMainData(teammaindata) {
    ...
    teammaindata.map((data,j)=> {
      teamData.push({
        ...
        roleName: data.roleName,                               //职级
        concatNumber: data.concatNumber,                       //当月联系
        directRegisterNumber: data.directRegisterNumber,       //团队当月分配
        ...
        yesterdayRegisterNumber: data.yesterdayRegisterNumber,
        yesterdayPromoteNumber: data.yesterdayPromoteNumber,
        yesterdayRenewNumber: data.yesterdayRenewNumber,
        totalYearsNumber: data.totalYearsNumber
      })
    })
    this.setState({
      teammaindata,
    ...
    const columns = [
      {
        ...
        render: (text, record) => {
          return (
            <div>
              {
                '昨日注册:' + record.yesterdayRegisterNumber
              }
            </div>
          )
        }
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              {
                '昨日晋升:' + record.yesterdayPromoteNumber
              }
            </div>
          )
        }
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              {
                '昨日续费：'+ record.yesterdayRenewNumber
              }
            </div>
          )
        }
                ...
                <div className="innerBox">
                  {
                    <div>
                      {
                        '直接注册:' + record.directRegisterNumber
                      }
                    </div>
                  }
      ...
      // },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              {
                '直接晋升:' + record.directPromoteNumber
              }
            </div>
          )
        }
      ...
      // },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              {
                '转介绍注册:'  + record.indirectRegisterNumber
              }
            </div>
          )
        }
      },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              {
                '转介绍晋升:'  + record.indirectPromoteNumber
              }
            </div>
          )
        }
  ...
  getTableData = (order) => {
    let dataList = []
      dataList.push({
        ...
        concatNumber: order.concatNumber,                       //团队当月已联系
        indirectRegisterNumber: order.indirectRegisterNumber,   //团队当月转介绍注册
        indirectPromoteNumber: order.indirectPromoteNumber,     //团队当月转介绍晋升
        yesterdayPromoteNumber: order.yesterdayPromoteNumber,
        yesterdayRegisterNumber: order.yesterdayRegisterNumber,
        yesterdayRenewNumber: order.yesterdayRenewNumber,
        // yesterdayRegisterNumber: order.yesterdayRegisterNumber,
        // yesterdayPromoteNumber: order.yesterdayPromoteNumber,
        // yesterdayRenewNumber: order.yesterdayRenewNumber,
        totalYearsNumber: order.totalYearsNumber
      })
    return dataList
  }
  ...
  requestImport = () => {
    ...
    base = base + 'beginTime=' + this.state.beginTime + '&endTime=' + this.state.endTime + '&type=2'
    console.log(base)
          ...
          >商品返点导出</Button> */}
          {
            ...
            <div>
              ...
              <Pagination
                ...
                showTotal={total => `共${this.state.dataTotal}个转化团队`}
              />
            </div>
          </Card>
        </Spin>
