...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      // 接收全部信息供页面展示
      // this.visitor = this.$route.query.visitor
      this.visitor = storage.get(EDIT_ALLINFO)
    },
    mounted() {
    ...
    methods: {
      ...
      goEditVisitorInfo(visitork,index) {
        // 设置订单信息进入缓存
        storage.set(EDIT_ALLINFO,this.visitor)
        storage.set(EDIT_KEYINFO,visitork)
        
        this.$router.push({
          path: '/editinfo',
          query: {
            // all: this.visitor,  //全部订单信息
            // visit: visitork,    //修改的订单信息
            num:index,             //修改的订单位置
          }
        })
      },
      ...
      savevisitorInfo() {
        ...
        savevisitor(params).then((res) => {
            ...
            if(res.code == '1'){
              ...
              this._showTips('保存出行人信息失败,请核查信息类型')
            }
        })
      },
