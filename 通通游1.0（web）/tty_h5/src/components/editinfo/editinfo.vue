<template>
  <section class="editVisitorInfo">
    ...
    <div class="peoInfo">
        ...
        <!-- <div class="pack item" v-show="this.keyidID == '护照'"> 
          ...
          <div class="email item" v-show="this.visitor.IDid == '2'"> 
        ...
        <div class="packnum item" v-show="this.visitor.IDid == '2'">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
        //取得订单信息
        // this.visitorinfo = this.$route.query.visit
        this.visitorinfo = storage.get(EDIT_KEYINFO)  //修改的
        this.alldata = storage.get(EDIT_ALLINFO)      //全部

        // this.visitorinfo.idIndate = this.visitorinfo.idIndate.Substring(0,10)

        //填写获取的预设订单信息
        this.visitor.orderDbid = this.visitorinfo.dbId, //用户id
        this.visitor.userName = this.visitorinfo.clientName //姓名
        this.visitor.milyName = this.visitorinfo.clientFamilyName  //证件姓
        ...
        this.visitor.email = this.visitorinfo.email            //手机号************
    ...
    methods: {
      ...
      back() {
        this.$router.push({
          path:  '/travellist',
          query: {
            // visitor: this.$route.query.all  //改为取本地缓存
          }
        })
      },
      _isValidDate(data){
      ...
      save(){
        ...
        // if (!this._isValidDate(this.visitor.birthday)){
        //   this._showTips('生日日期格式不正确')
        //   return
        // }
        ...
        this.visitorinfo.phone = this.visitor.phone            //手机
        this.visitorinfo.email = this.visitor.email            //邮箱
        this.visitorinfo.birthDate = this.visitor.birthday      //出生日期
        ...
        this.visitorinfo.idIndate = this.visitor.IDEndDate       //证件有效期
        ...
        // 修改订单指定出行人的信息
        // console.log(this.alldata.detail.length)
        var i
        for(i= 0; i<this.alldata.detail.length;i++){
            this.alldata.detail[this.$route.query.num] = this.visitorinfo
        }

        // console.log('修改后的全部信息')
        // console.log(this.alldata)

        // 替换缓存全部订单信息为当前修改后的全部信息
        storage.set(EDIT_ALLINFO,this.alldata)  

        this.$router.push({
          path:  '/travellist',
          query: {
            // visitor: this.alldata     //传回全部的修改后的订单信息
          }
        })
