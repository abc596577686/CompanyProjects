<template>
    <div class="calendar">
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click.stop.prevent="select(k1,k2,$event)">
                <span :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}">{{child.day}}</span>
                <pre class="text" v-if="child.eventName!=undefined">{{child.eventName}}</pre>
...
export default {
    ...
    methods: {
        ...
        select(k1, k2, e) {
            ...
            // if (dom.tagName == 'TD'){
            //     dom.style.backgroundColor = '#FFA83A';
            // }
            // if (dom.tagName == 'SPAN' || dom.tagName == 'PRE'){
            //     console.log(dom.parentNode);
            //     dom.parentNode.style.backgroundColor = '#FFA83A';
            // }
...
<style lang="css" scoped rel="stylesheet/css">
...
.calendar tr{
    border-bottom: 0.042667rem solid #f5f5f5;
}
.calendar td {
...
/*.calendar td.selected:not(:first-child){
    background: #ccc;
}*/
/*.calendar td.selected:first-child{
