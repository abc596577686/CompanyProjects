    ...
    <div>
        ...
        <div v-show="!showLoading">
            ...
            <div class="info_div">
                ...
                <!-- 普通商品价格 -->
                <p class="price" v-show="productInfo.isGroupProduct == 0 && isSeckillReadStatus == 0" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.price}} <del class="price_del" v-show="productInfo.taobaoPrice != ''">¥{{productInfo.taobaoPrice}}</del> </p>
                <p class="price" v-show="productInfo.isGroupProduct == 0 && isSeckillReadStatus == 1" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.originalPrice}} <del class="price_del" v-show="productInfo.taobaoPrice != ''">¥{{productInfo.taobaoPrice}}</del> </p>
                <!-- 组合商品价格 -->
    ...
    methods: {
        ...
        _seckillProductDown: function(){
            ...
            setInterval(function(){
                ...
                }else if(seckillStartTime < currentTimes && currentTimes < seckillEndTime){
                    // 秒杀商品进行中
                    let currentDate = new Date(currentTimes);
                    let seckillEndDate = new Date(seckillEndTime);
        ...
        add(imposeBuyData) {
            ...
            if(this.showProductSpec == true){  //规格商品
                console.log(imposeBuyData)
                console.log('当前选择数量'+this.addcartpram.addNum)   
                
                if (imposeBuyData.isSeckillLimit == 0) {  //非限购商品
                    if (this.addcartpram.addNum > imposeBuyData.stockNumber) {
                        this.showAlert=true;
                        this.alertText="库存不足";
                        this.addcartpram.addNum = Number(this.addcartpram.addNum) - 1
                    }
                }else if (imposeBuyData.isSeckillLimit == 1) {  //限购商品
                    imposeBuyData.seckillLimitNumber = Number(imposeBuyData.seckillLimitNumber)
                    ...
                    if(imposeBuyData.seckillLimitNumber > imposeBuyData.stockNumber){   //限购数大于库存
                        if(this.addcartpram.addNum > imposeBuyData.stockNumber){  //选择数大于库存数
                            this.showAlert=true;
                            this.alertText="库存不足";
                            this.addcartpram.addNum = Number(this.addcartpram.addNum) - 1
                        }
                    }else if(imposeBuyData.seckillLimitNumber < imposeBuyData.stockNumber){
                        if(this.addcartpram.addNum > imposeBuyData.seckillLimitNumber){  //限购数小于库存
                            this.showAlert=true;
                            this.alertText="已达最大购买数量";
                            this.addcartpram.addNum = Number(this.addcartpram.addNum) - 1
                        }
                    }else if(imposeBuyData.seckillLimitNumber == imposeBuyData.stockNumber){
                        if(this.addcartpram.addNum > imposeBuyData.seckillLimitNumber){  //限购数等于库存
                            this.showAlert=true;
                            this.alertText="已达最大购买数量";
                            this.addcartpram.addNum = Number(this.addcartpram.addNum) - 1
                        }
