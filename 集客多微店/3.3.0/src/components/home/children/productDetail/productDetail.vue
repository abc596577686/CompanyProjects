<template>
    <div>
        ...
        <div v-show="!showLoading">
            <!-- 多图 -->
            <nav class="msite_nav">
                <more-banner  v-if="headerImageList.length >1" :bannerList="headerImageList"></more-banner>
                <!-- 单图 -->
                <img v-if="headerImageList.length == 1" class="singleimg" :src="headerImageList[0].imageUrl">
            </nav>
            ...
            <!-- </div> -->
            <!-- 链接失效提示 -->
            <div class="lossCt" v-show="productInfo.isExpire == 1">
                <p class="lossCt_txt" >您查看的商品链接已失效，请重新获取链接</p>
                <!-- 您查看的商品链接已失效，请重新获取链接 -->
            </div>
            <div class="info_div">
                ...
                <!-- 普通商品价格 -->
                <!-- <p class="price" v-show="productInfo.isGroupProduct == 0 && productInfo.isSeckillReadStatus == 0" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.price}}  </p> -->
                <!-- <p class="price" v-show="productInfo.isGroupProduct == 0 && productInfo.isSeckillReadStatus == 1" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.originalPrice}}  </p> -->
                <!-- 组合商品价格 -->
                <!-- <p class="price" v-show="productInfo.isGroupProduct == 1" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.groupPriceRegion}}</p> -->

                <!-- 普通商品 -->
                <p class='price' v-show="productInfo.isGroupProduct === '0' && productInfo && productInfo.isSpceProduct != 1" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.priceRegion}}</p>
                <p class='price' v-show="productInfo.isGroupProduct === '0' && productInfo && productInfo.isSpceProduct == 1 " style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.priceRegion}}</p>
                <!-- 组合商品  -->
                <!-- <p class='price' v-show="productInfo.isGroupProduct === '1' && productInfo.groupPriceRegion" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.groupPriceRegion}}</p> -->
                <p class='price' v-show="productInfo.isGroupProduct === '1' && productInfo.groupPriceRegion" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.priceRegion}}</p>
            ...
            <div class="list-dp">
                ...
                <div class="weui-cell weui-cell-access" style="height:50px">
                    <div class="weui-cell-bd" @click="cpa()">
                        <span style="vertical-align: middle"><p class="dp"><span style="width:2.8rem;display:inline-block">运费</span><span id="yunfei">{{postage}}</span></p>
                        </span>
                        <div class="weui-cell-ft" style="position:relative;bottom:5px"></div>
                    </div>
                </div>
                <!-- <div class="weui-cell weui-cell-access" style="height:50px" v-if="productInfo.productType == 2">
                    <div class="weui-cell-bd" @click="jksTit()">
                        <span style="vertical-align: middle"><p class="dp"><span style="width:2.8rem;display:inline-block">进口税</span><span id="yunfei">{{inage}}</span></p>
                        </span>
                        <div class="weui-cell-ft" style="position:relative;bottom:5px"></div>
                    </div>
                </div> -->
            ...
            <transition name="spec_mandel_ani">
                <section class="choose_type_Container choose_spec_container" v-if="showBuy">
                    ...
                    <div class="proTitle">
                        ...
                        <span class="proPrice">¥{{currentProductPrice}}</span>
                    ...
                    <div class="list" style="position: relative;">
                        ...
                        <div class="gw_num" style="margin-top: 13px;">
                            ...
                            <span class="input-number-increment" @click="add(imposeBuyData);">+</span>
                        </div>
                        <div class="warntit" v-if="!showProductSpec && productInfo.isSeckillLimit == 1 && productInfo.seckillExistBuyNum != 0">
                            ...
                            当前您最多可购买{{productInfo.seckillLimitNumber-productInfo.seckillExistBuyNum}}件此规格商品
                        </div>
                        <div class="warntit" v-if="!showProductSpec && productInfo.isSeckillLimit == 1 && productInfo.seckillExistBuyNum == 0">
                        ...
                        <div class="warntit" v-if="showProductSpec && imposeBuyData.isSeckillLimit == 1 && imposeBuyData.seckillExistBuyNum != 0">
                            ...
                            当前您最多可购买{{imposeBuyData.seckillLimitNumber-imposeBuyData.seckillExistBuyNum}}件此规格商品
                        </div>
                        <div class="warntit" v-if="showProductSpec && imposeBuyData.isSeckillLimit == 1 && imposeBuyData.seckillExistBuyNum == 0">
                    ...
                    <!-- <div class="shuifei" v-if="productInfo.productType == 2">
                        <div class="shuifei1">税费</div>
                        <div class="shuifei2">{{'￥'+taxnum}}</div> 
                    </div>  -->
...
<!-- 
            ...
            <footer class="foot_div" style="z-index:10000;height:2.1rem">
                ...
                <div class="href" @click="goOrderlist">
                    <div class="gwc ff" style="background:#fff">
                        <!-- <p class="addcart-goods-num" v-show="quantity>0">{{quantity}}</p> -->
                        <img src="../../../../common/images/kkop.png"> 我的订单
                    </div>
                </div>
                ...
                <div class="actionGroup">
                    <button class="bt full" v-show="productInfo.isExpire == 1" style="width: 55%;background: #999;">商品链接已失效</button>
                    <!-- <button class="jr_gwc bt part"  v-show="productInfo.isSeckillProduct == '0' && productInfo.isSoldOut!='1'" @click="addCartEnv">加入购物车</button>  -->
                    <button class="gm bt part"  v-show="productInfo.isSeckillProduct == '0' && productInfo.isSoldOut!='1' && productInfo.isExpire == 0"  @click="buyNowEnv">立即购买</button> 
                    <button class="bt full" v-show="productInfo.isSeckillProduct == '0' && productInfo.isSoldOut=='1' && productInfo.isExpire == 0" style="width: 55%;background: #999;">商品已售罄</button>  
                    <!--秒杀商品  -->
                    <!--开抢提醒  status=1 -->
                    ...
                    <button class="gm bt part"  v-show="productInfo.isSeckillProduct == '1' && productStatus == 2 && productInfo.isExpire == 0"  @click="buyNowEnv">立即购买</button>
                    <!--提示升级  status=3-->
                    ...
                    <!--提示售罄  status=4-->
                    <button class="bt full" v-show="productInfo.isSeckillProduct == '1' && productStatus == 4 && productInfo.isExpire == 0" style="width: 55%;background: #999;">商品已售罄</button>
                </div>
            </footer>
        </div>
        ...
        <transition name="slide">
            <section v-if="yunfeikk" class="choose_type_Container" style="height:13rem">
                ...
                <div class="gm-task" > 
                    <div class="sm_right" style="margin-left: 10px;margin-top: 0px;padding-top:8px;padding-right:10px" v-for="data in taxDescList">
                        <div style="color:#666;font-size:0.55rem;"><span></span>{{data.areaNames}}</div>
                        <div style="color:#666;font-size:0.5rem;line-height:18px;margin-left: 11px;margin-top: 2px;padding-right:15px">{{data.priceNotice}}</div>
                    </div>
                </div>
        ...
        <!-- 进口税说明 -->
        <transition name="slide">
            <section v-if="yunfeikk2" class="choose_type_Container" style="height:13rem;">
                <div class="freightDes">
                    <div class="fD_1">
                        税费说明
                    </div>
                    <span class="fD_2" @click="yunfeiXs2()">
                        <img src="../../../../common/images/icon_guanbi@2x.png">
                    </span>
                    <!-- <div class="weui-cell-ft"></div> -->
                </div>
                ...
                <div class="description">
                   <div class="gm-task" > 
                        <div class="sm_right" style="margin-left: -8px;margin-top: 0px;padding-top:8px" v-for="data in taxlist">
                            <!-- <div style="color:#666;font-size:0.55rem;line-height:1rem;"><span></span>{{data.content}}</div> -->
                            <div style="color:#666;font-size:0.55rem;line-height:1rem;">{{data.content}}</div>
                            <!-- <div style="color:#666;font-size:0.5rem;line-height:18px;margin-left: 11px;margin-top: 2px;padding-right:15px">{{data.priceNotice}}</div> -->
                        </div>
                    </div>
                </div>
        ...
        <transition name="fade">
            <span class="grey1" v-if="yunfeikk || iszhuce || showBuy || showChose || showDesc || yunfeikk1 || yunfeikk2" @click="hideMask"></span>
        </transition>
...
import action from '../../../../crypto/index.js'
...
export default {
  ...
  created() {
    ...
    // console.log(this.$route.query.shareUserSecret)
  },
  mounted() {
    ...
    // let keyword = [this.productId,this.shareTime,this.$route.query.shareUserSecret]
    // console.log(JSON.parse(localStorage.SaveToken)) 
    // localStorage.setItem('SaveToken','')

    let localdata = []
    if(localStorage.SaveToken){
      localdata = localStorage.SaveToken
      let pushdata = [this.productId,this.shareTime,this.$route.query.shareUserSecret]
      localdata = JSON.parse(localdata)
      if(localdata.length == 15){  //超出15条记录清空本地缓存
        localdata = []
        let pushdat = [this.productId,this.shareTime,this.$route.query.shareUserSecret]
        localdata.push(pushdat)
        localStorage.setItem('SaveToken',JSON.stringify(localdata))
        if(!this.$route.query.shareUserSecret){
          localStorage.setItem('shareUserSecret','empty')
        }else{
          localStorage.setItem('shareUserSecret',JSON.stringify(this.$route.query.shareUserSecret))
        }
        ...
        for(let i=0;i<localdata.length;i++){
          ...
          if(localdata[i][0] == this.productId){
            if(localdata[i][1] == this.shareTime){  //查询当前链接的 商品id/分享时间 找到精确目标token
              console.log(i)
              if(!this.$route.query.shareUserSecret){
                localStorage.setItem('shareUserSecret','empty')
              }else{
                localStorage.setItem('shareUserSecret',JSON.stringify(localdata[i][2]))
              }
              this._getProductDetail();
              return
            }
      ...
      let pushdata = []
      let pushdat = [this.productId,this.shareTime,this.$route.query.shareUserSecret]
      pushdata.push(pushdat)
      localStorage.setItem('SaveToken',JSON.stringify(pushdata))
      if(!this.$route.query.shareUserSecret){
        localStorage.setItem('shareUserSecret','empty')
      }else{
        localStorage.setItem('shareUserSecret',JSON.stringify(this.$route.query.shareUserSecret))
    ...
    // }
    // else{
    //    localStorage.shareUserSecret = JSON.stringify(this.$route.query.shareUserSecret)
    // }

    // let keytxt = '10bb2a5636dc0b06a6e6f2438dd09ce1'
    // this.decrypt(keytxt)
  ...
  methods: {
    // decrypt(keytxt){
    //   this.decryptMsg = action.Decrypt(keytxt);
    //   console.log(keytxt)
    //   console.log('解密后：' + this.decryptMsg)
    // },
    showusemethod() {
    ...
    _getProductDetail() {
      ...
      productDetail(params).then(res => {
        ...
        this.taxDescList = res.data.newPostageData.postageDescList;
        this.inage = res.data.taxData.taxInfo;
        this.taxlist = res.data.taxData.taxDescList;
        this.taxnum = res.data.taxData.tax;
        this.taxnum1 = res.data.taxData.tax;
        this.isSpec = res.data.isSpceProduct;
        this.isSeckillLimit = res.data.isSeckillLimit;

        // if(res.data.isSpceProduct == 1){
        //     this.taxnum = "0.00"
        // }else{
        //     this.taxnum = res.data.taxData.tax
        // }
        ...
        this.taxnum = res.data.taxData.tax;
        this.localtax = res.data.taxData.tax;
        ...
        if (res.code == "1") {
          ...
          if (me.productInfo) {
            ...
            me.currentProductPrice = me.productInfo.priceRegion;
          ...
          if (me.productInfo.isSpceProduct == 1) {
            ...
            me.productSpecList.forEach((spec, index) => {
              ...
              if (toggle) {
                me.primarySpecList.push(spec);
                me.primarySpecModelName =
                  me.primarySpecList[0].primarySpecModelName;
              }
            });
          }
          ...
          // 运费 优先判断是否包邮
          if (res.data.isFreePostage == 1) {
            //包邮
            if (res.data.newPostageData.postage == "") {
              me.postage = "";
            } else {
              me.postage = res.data.newPostageData.postage + "元";
            }
            // this.kkk = res.data.postageData.postageDescH5List
          } else if (res.data.isFreePostage == 0) {
            //不包邮
            if (res.data.newPostageData.postage == "") {
              me.postage = "";
            } else {
          ...
          if (res.data.isSeckillProduct == "1") {
            //是秒杀商品
            if (me.seckillStartTime >= me.currentTimes) {
              //活动预热中
              if (this.productInfo.isBuyPackage == "0") {
                //经销商
                if (this.productInfo.isSoldOut == "0") {
                  //有货
          ...
          sa.track("commodityDetail", {
            ...
            firstCommodity: me.primarySpecList.length
              ? me.primarySpecList.join("")
              : "",
            secondCommodity: me.secondarySpecList.length
              ? me.secondarySpecList.join("")
              : "",
            ...
            commodityDiscountAmount:
              parseFloat(me.productInfo.taobaoPrice) -
              parseFloat(me.productInfo.price)
          });
    ...
    _seckillProductDown: function() {
      ...
      setInterval(function() {
        ...
        // console.log(me.currentTimes - seckillStartTime)
        // console.log(seckillStartTime)
        // console.log(me.currentTimes)
        // console.log(currentTimes)

        // console.log(seckillStartTime - me.currentTimes)
        // console.log(seckillStartTime)
        // console.log(seckillStartTime)
        // console.log(seckillEndTime)

        if (currentTimes < seckillStartTime) {
          // 秒杀商品进行中
          let currentDate = new Date(currentTimes);
          let seckillEndDate = new Date(seckillEndTime);
          let seckillStartDate = new Date(seckillStartTime);

          let t = seckillStartDate.getTime() - currentDate.getTime();
          ...
          if (hour == 0 && min == 0 && sec == 1) {
            setTimeout(me._getProductDetail(), 2000);
          }
        } else if (
          seckillStartTime < currentTimes &&
          currentTimes < seckillEndTime
        ) {
          // 秒杀商品进行中
          let currentDate = new Date(currentTimes);
          let seckillEndDate = new Date(seckillEndTime);
    ...
    addCartEnv() {
      this.iszhuce = false;
      if (this.productInfo.isH5CommonUser == 1) {
        //加入购物车优先判断注册
        this.iszhuce = true;
      } else {
        this.oprType = 1;
        ...
        if (this.productInfo.isGroupProduct == ASSEMBLY_PRODUCT) {
          // 商品组合
          this.$router.push({
    ...
    chooseProductNum() {
      ...
      this.currentProductPrice = this.productInfo.priceRegion
    ...
    add(imposeBuyData) {
      ...
      if (this.isSpec == 0) {
        this.taxnum = (this.taxnum1 * this.addcartpram.addNum).toFixed(2);
      } else {
        this.taxnum = (this.numtax * this.addcartpram.addNum).toFixed(2);
      }

      if (this.showProductSpec == true) {
        //规格商品
        console.log(imposeBuyData);
        console.log("当前选择数量" + this.addcartpram.addNum);

        if (imposeBuyData.isSeckillLimit == 0) {
          //非限购商品
          if (this.addcartpram.addNum > imposeBuyData.stockNumber) {
            this.showAlert = true;
            this.alertText = "库存不足";
            this.addcartpram.addNum = Number(this.addcartpram.addNum) - 1;
          }
        } else if (imposeBuyData.isSeckillLimit == 1) {
          //限购商品
          imposeBuyData.seckillLimitNumber = Number(
            imposeBuyData.seckillLimitNumber
          );
          imposeBuyData.stockNumber = Number(imposeBuyData.stockNumber);

          console.log(imposeBuyData.seckillLimitNumber);
          console.log(imposeBuyData.stockNumber);

          if (imposeBuyData.seckillLimitNumber > imposeBuyData.stockNumber) {
            //限购数大于库存
            if (this.addcartpram.addNum > imposeBuyData.stockNumber) {
              //选择数大于库存数
              this.showAlert = true;
            ...
            imposeBuyData.seckillLimitNumber < imposeBuyData.stockNumber
          ) {
            if (this.addcartpram.addNum > imposeBuyData.seckillLimitNumber) {
              //限购数小于库存
              this.showAlert = true;
              this.alertText = "已达最大购买数量";
              this.addcartpram.addNum = Number(this.addcartpram.addNum) - 1;
            }
          } else if (
            imposeBuyData.seckillLimitNumber == imposeBuyData.stockNumber
    ...
    selectPrimarySpec(item, index) {
      ...
      if (this.primarySpecList.length > 0) {
        ...
        if (this.productInfo.isMoreSpecProduct == 1) {
          ...
          this.productSpecList.forEach(productSpec => {
            if (
              productSpec.primarySpecId == item.primarySpecId &&
            ...
            if (tmpSecondarySpecList.length > 0) {
              tmpSecondarySpecList.forEach(tmpProductSpec => {
                if (
                  ...
                  productSpec.secondarySpecId = tmpProductSpec.secondarySpecId;
                  productSpec.secondarySpecName =
                    tmpProductSpec.secondarySpecName;
                  productSpec.stockNumber = tmpProductSpec.stockNumber;
                  productSpec.paymentPrice = tmpProductSpec.price;
                  productSpec.tax = tmpProductSpec.tax;

                  if (productSpec.isChecked == true) {
                    console.log(tmpProductSpec);
                    this.taxnum = tmpProductSpec.tax;
                    this.numtax = tmpProductSpec.tax;
                    // this.productPrice = productSpec.price;
                    this.productPrice = tmpProductSpec.price;

                    this.currentProductPrice = tmpProductSpec.price;
      ...
      if (this.productInfo.isMoreSpecProduct == 0) {
        ...
        this.currentProductPrice = item.price;
        console.log(item);
        this.taxnum = item.tax;
        this.numtax = item.tax;
    ...
    selectSecondarySpec(item, index) {
      ...
      if (this.secondarySpecList.length > 0) {
        ...
        this.productSpecList.forEach(productSpec => {
          if (
            productSpec.secondarySpecId == item.secondarySpecId &&
            productSpec.isSoldOut == 0
          ) {
            ...
            tmpPrimarySpecIdList.forEach(primarySpecId => {
              if (productSpec.primarySpecId == primarySpecId) {
                ...
                if (productSpec.isChecked == true) {
                  ...
                  // this.productPrice = item.price;
                  this.productPrice = item.paymentPrice;

                  this.currentProductPrice = item.paymentPrice;
      ...
      if (primarySpecName != null) {
        this.selectSpecInfo =
          "已选择：" + primarySpecName + "、" + item.secondarySpecName;
      }
      item.isChecked = true;
    ...
    getTop() {
      window.location.href =
        "http://a.app.qq.com/o/simple.jsp?pkgname=com.webapp.jkd";
    },
    // 监听高度方法
    handleScroll() {
      var scrollTop =
    ...
    settime(obj) {
      ...
      if ((this.timeoutKey == "1") & (this.linkKey != "1")) {
        //倒计时控制开启  接口控制关闭  此时进行倒计时
        this.disabled = true;
    ...
    ktload() {
      ...
      if (this.productInfo.isH5CommonUser == 1) {
        //需要注册
        this.iszhuce = true;
      } else if (this.productInfo.isH5CommonUser == 0) {
        //无需注册 直接前往
        me.$router.push({
            ...
            wx.onMenuShareTimeline({
              ...
              success() {
                ...
                sa.track("shareCommodity", {
                  ...
                  firstCommodity: _this.primarySpecList.length
                    ? _this.primarySpecList.join("")
                    : "",
                  secondCommodity: _this.secondarySpecList.length
                    ? _this.secondarySpecList.join("")
                    : "",
            ...
            wx.onMenuShareAppMessage({
              ...
              success: function() {
                ...
                sa.track("shareCommodity", {
                  ...
                  originalPrice: parseFloat(_this.productInfo.taobaoPrice),
                  // pricePerCommodity: parseFloat(_this.productInfo.price),
                  pricePerCommodity: parseFloat(_this.productInfo.priceRegion),
  ...
  watch: {
    ...
    num(newValue, oldValue) {
      ...
      // 当前所有筛选下库存最小值
      if (this.productInfo.isSpceProduct == 0) {
        //非规格商品
        if (this.productInfo.isSeckillLimit == 0) {
          //非限购商品
          ...
          //限购商品
          if (
            this.productInfo.seckillLimitNumber -
              this.productInfo.seckillExistBuyNum ==
            0
          ) {
            //初始已无可购数量
            this.addcartpram.addNum = 1;
            ...
            //超出限购数量
            if (this.productInfo.seckillExistBuyNum != 0) {
              //购买过该商品
              // this.showAlert=true;
              this.addcartpram.addNum =
                this.productInfo.seckillLimitNumber -
                this.productInfo.seckillExistBuyNum;
            } else {
              //未购买过该商品
        ...
        //规格商品
        if (this.imposeBuyData.isSeckillLimit == 0) {
          //非限购商品
          if (newValue > this.stockNumber) {
            this.showAlert = true;
            this.alertText = "温馨提示,超出商品库存限额~";
          ...
          //限购商品
          if (
            this.imposeBuyData.seckillLimitNumber -
              this.imposeBuyData.seckillExistBuyNum ==
            0
          ) {
            //初始已无可购数量
            this.addcartpram.addNum = 1;
            ...
            //超出限购数量
            if (this.imposeBuyData.seckillExistBuyNum != 0) {
              //购买过该商品
              // this.showAlert=true;
              this.addcartpram.addNum =
                this.imposeBuyData.seckillLimitNumber -
...
.proColor button {
  ...
  border: 0.042667rem solid #999999;
...
.ee {
  border: 0.042667rem solid #e5e5e5 !important;
