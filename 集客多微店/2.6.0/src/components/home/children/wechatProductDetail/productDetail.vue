    ...
    <div>
        ...
        <div v-show="!showLoading">
            ...
            <div class="info_div">
                ...
                <!-- <p class="coverwww" style="font-size:15px;height: 11px;background-color: #f5f5f5; z-index: 1999999;position: absolute;"></p> -->
                <!-- 普通商品价格 -->
            </div>
            <!-- <p class="coverwww" style="font-size:15px;height: 11px;background-color: #f5f5f5; z-index: 1999999"></p> -->
            <div class="info_div">
              <p class="price" v-show="productInfo.isGroupProduct == 0 && isSeckillReadStatus == 0" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.price}}  </p>
              <p class="price" v-show="productInfo.isGroupProduct == 0 && isSeckillReadStatus == 1" style="color:#FB4A4C;font-size:20px;font-weight:600">¥{{productInfo.originalPrice}}  </p>
              <!-- 组合商品价格 -->
            ...
            <div class="chooses">
                <div class="left c">
                    <div class="kkos" :class='{chcos: proStatus ==0}' @click="showProductChange(0)">同类热销</div>
                </div>
                <div class="right c">
                    <div class="kkos" :class='{chcos: proStatus ==1}' @click="showProductChange(1)">为你推荐</div>
                </div>
            </div>
            <!-- 商品展示 -->
            <!-- <div class="productShow" style="width:100%" >
              <swiper :options="swiperOption"  v-if="proStatus == 0">
                  <swiper-slide v-for="items in sameKindWholesaleList" class="showInfo">
                    <div class="imageWrap">
                      <img :src="items.imageUrl" class="goodsImage">
                    </div>
                    <div class="goodsText">{{items.name}}</div>
                    <div class="showPrice">{{items.price?'￥'+items.price:''}}</div>
                  </swiper-slide>
              </swiper> -->
              <!-- <swiper :options="swiperOptions" v-if="proStatus == 1">
                  <swiper-slide v-for="items in recommendWholesaleList" class="showInfo">
                    <div class="imageWrap">
                      <img :src="items.imageUrl" class="goodsImage">
                    </div>
                    <div class="goodsText">{{items.name}}</div>
            ...
            <!-- 商品推荐 -->
            <div class="productShow" style="overflow-x: scroll;" v-if="proStatus == 0">
              <div v-for="items in sameKindWholesaleList" class="showInfo">
                <div class="imageWrap" @click="goallevaluate(items)">
                  <img :src="items.imageUrl" class="goodsImage">
                </div>
                <div class="goodsText" style="-webkit-box-orient: vertical"  @click="goallevaluate(items)">{{items.name}}</div>
                <div class="showPrice"  @click="goallevaluate(items)">{{items.price?'￥'+items.price:''}}</div>
              </div>
            </div>
            <div class="productShows" style="overflow-x: scroll;" v-if="proStatus == 1">
              <div v-for="items in recommendWholesaleList" class="showInfo" >
                <div class="imageWrap"  @click="goallevaluate(items)">
                  <img :src="items.imageUrl" class="goodsImage">
                </div>
                <div class="goodsText" style="-webkit-box-orient: vertical"  @click="goallevaluate(items)">{{items.name}}</div>
            ...
            <section v-if="yunfeikk" class="choose_type_Container" style="height:13rem">
                ...
                <div class="gm-task" > 
                    <div class="sm_right" style="margin-left: 10px;margin-top: 0px;padding-top:8px;padding-right:10px" v-for="data in taxDescList">
                        <div style="color:#666;font-size:0.55rem;"><span></span>{{data.postageDescH5}}</div>
...
export default {
  data() {
    return {
      ...
      decryptMsg: "",
      shareTime: "",
      sameKindWholesaleList: [], //同类推荐
      recommendWholesaleList: [] //为你推荐
    };
  },
  ...
  mounted() {
    ...
    if (localStorage.SaveToken) {
      ...
      let pushdata = [
        this.productId,
        this.shareTime,
        this.$route.query.shareUserSecret
      ];
      ...
      if (localdata.length == 15) {
        ...
        if (!this.$route.query.shareUserSecret) {
          ...
          localStorage.setItem(
            "shareUserSecret",
            JSON.stringify(this.$route.query.shareUserSecret)
          );
        }
        ...
        for (let i = 0; i < localdata.length; i++) {
          ...
          if (localdata[i][0] == this.productId) {
            if (localdata[i][1] == this.shareTime) {
              //查询当前链接的 商品id/分享时间 找到精确目标token
              console.log(i);
              if (!this.$route.query.shareUserSecret) {
                localStorage.setItem("shareUserSecret", "empty");
              } else {
  ...
  methods: {
    ...
    goallevaluate(item) {
      ...
      this._getProductDetail();
      // this.$router.push({
      //   path: this.routerPath + "/wechatProductDetail",
      //   query: {
      //     a: "v1/productDetailCustoms",
    ...
    _getProductDetail() {
      ...
      me.showLoading = true;
      wechatProductDetail(params).then(res => {
        console.log("商品详情数据----");
        console.log(res);
        ...
        this.taxDescList = res.data.postageData.postageDescH5List;
        ...
        this.sameKindWholesaleList = res.data.sameKindWholesaleList;
        this.recommendWholesaleList = res.data.recommendWholesaleList;
    ...
    showProductChange(state) {
      this.proStatus = state;
    },
    doOperation() {
      //加入购物车或者立即购买
      let me = this;
    ...
    _wxShareEnv() {
      ...
      if (isWeiXin()) {
        ...
        wechatProductDetail(xparams).then(res => {
          if (res.code == "1") {
            link = res.shareURL;
